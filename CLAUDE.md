...
### 22. Recurring license activation → Xero invoice from the deploy page (HIGH PRIORITY — top of the list)
- **Decided 2026-09-18** (design session). Recurring license items are **recurring revenue, not project COGS**. Knack does NOT track the ongoing subscription — Xero does. Knack's only job is to anchor **when the client's subscription invoice gets created** (the moment the PM turns the service on) and to tie that purchase back to the exact proposal + CO license lines it covers.
- **The problem**: licenses (product bucket `licenses`, id `645554dce6f3a60028362a6a`; `view_3371` on the proposal page; shipped to Make as `licenseIds`) are the line most likely to change via CO, and the vendor doesn't bill until the system is plugged in. Creating their expense/invoice at acceptance or at greenlight mismatches income and expense, and the "find those invoices and re-code them to a different account than equipment/install" step is exactly the nuance to avoid. Whether the PM buys original + CO licenses in one order or two must not matter.
- **Locked decisions**:
  1. **No new objects.** Reuse the INSTALL line items (same object as `view_4093` / `view_4056`). Three new fields, used only by the licenses bucket: `activation date` (date), `vendor purchase receipt` (file), `Xero invoice link` (link). "Pending" = activation date empty.
  2. **Greenlight conversion must include licenses.** The Make scenario that converts the accepted proposal into install line items must ALSO create install lines for the proposal's recurring license records (`view_3371` / `licenseIds`) — today only equipment/install lines convert. Same for COs signed after greenlight: the CO-signed scenario creates install lines for license adds; a license remove cancels/decrements the matching pending license line.
  3. **Account coding lives on the product, not on the invoice.** Add a Xero account code (or item code) to the licenses-bucket Products so Make copies it onto each Xero line. Add a Xero contact id on the company record so Make addresses by id, never by name match.
  4. **UI = checkboxes + button + modal on the deploy scene.** A table of this project's licenses-bucket install lines with empty activation date → PM ticks the ones just purchased → "Record activation" → custom modal asks for the receipt PDF + activation date (default today; this becomes the Xero invoice date) and shows the total qty of the ticked lines as the sanity check. Never ask the PM to type a count — the checklist IS the match to proposal + CO lines. **Front end writes the lines** (activation date + receipt) for instant feedback, then refreshes the table so they drop out of pending.
  5. **Receipt uploads once.** Upload the PDF to the Knack asset endpoint one time, set the same asset id on every ticked line. TEST that Knack accepts one asset referenced from several records; if not, receipt goes on the first ticked line only.
  6. **One webhook per activation, after the writes finish**, carrying project id + activation date (NOT line ids). Make queries Knack for install lines on that project with that activation date and empty Xero link, builds ONE Xero invoice (contact id from company, account code from product, invoice date = activation date, receipt attached), writes the Xero link back to every line. The webhook is a nudge, not the source of truth — a partial front-end failure + retry just fires a second webhook that picks up the leftovers.
  7. **Double-invoice guards in two places**: the checklist only offers lines with an empty activation date; Make only invoices lines with an empty Xero link. Together a double click or re-fired webhook is harmless.
  8. **Xero repeating invoice preferred.** Ongoing billing lives in Xero, so create a Xero *repeating* invoice starting at the activation date if Make's Xero module supports it; otherwise create the first invoice via Make and set the repeating template up in Xero by hand (or Make HTTP module → Xero API). Knack never stores next-invoice / renewal dates.
  9. **Two dates, kept separate.** Activation date drives the client invoice. Vendor purchase date lives on the receipt and only matters for bookkeeping.
- **Explicitly rejected (don't re-litigate)**: a Subscription object with cadence/term/next-invoice/renewal fields (Knack isn't tracking the subscription); an Activation object (batching + receipt live on the lines instead); storing activation date / receipts in a config JSON blob (Knack view filters and the Make query can't see inside JSON, and JSON can't hold a file); Make as the sole writer of the lines (rejected for instant feedback — front end writes, Make invoices); Knack-side invoicing (invoicing happens in Xero).
- **Build order**: Builder fields (1 + 3) → Make greenlight + CO conversion incl. licenses (2) → deploy-scene checklist + modal + webhook in the bundle (4–6) → Make activation scenario (6–8).
- **Nearest siblings for the bundle piece**: `worksheet-v2/bulk.js` for the checkbox selection + toolbar and the concurrency-capped + retry queue the per-line PUTs must use; `acceptance-card.js` `openXeroEditor` for the modal idiom and the Xero-link field pattern (`field_1847` on the acceptance record); the ops-stepper webhook POST (`src/config.js` payload shape) for the Make call.
- **Builder TBDs**: field ids for the three new install-line fields, the Xero account-code field on Products, the Xero contact-id field on the company record, and the deploy-scene view that hosts the checklist.
