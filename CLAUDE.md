...
- **What survey HAS today**: `field_2380` (Connected Devices, multi, on NVR/switch) and `field_2381` ("Connected To", single, on cam/reader) exist as **plain Knack connections** — NOT backed by any cascade, and there is no accessory→parent field at all. So the v2 survey card intentionally renders **no** mounting-hardware/accessory UI and maps no `accessories`/`children`/`parent`/`accessoryMatch` logical slots (see the `view_3505` config entry + card branch).
