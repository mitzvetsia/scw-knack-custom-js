...
- **Decided 2026-09-11 (Known Issue #12)**: `field_2197` (and the survey/install
  twins `field_2381`/`field_2821`) will be DELETED — switch-side editing only,
  camera side derived in JS from the parents' forward lists. Until that lands,
  everything above still applies; don't build anything NEW that reads or
  writes the camera-side fields.
...
- **Progress (2026-08-03) — STEP 1 SHIPPED**: `field_2197` is now read-only in every UI surface. worksheet-v2: both `detailConnection(..., 'field_2197', ...)` call sites (cam detail + sales detail, `worksheet-v2/card.js`) pass `readOnly=true` (no `data-scw-ws-v2-conn` picker hook), and the `field_2197` conn-single entries were removed from `bulk.js` `FIELDS.cam` / `SALES_FIELDS.cam`. v1: the three `nativeEdit` configs in `device-worksheet.js` flipped to `readOnly`. Knack-native surfaces: new `src/features/connected-to-readonly.js` (global) kills grid inline edit on `td.field_2197.cell-edit` (CSS pointer-events + capture-phase blocker) and locks `#kn-input-field_2197` in Knack edit/create forms per the locked-field convention, with a hint pointing at the parent's Connected Devices picker. Display side already derives from the authoritative union (forward `field_1957` ∪ `field_2197` back-pointers) in worksheet-v2 (`init.js` `detailConnectedDevices` union) and bid-review-v2 (`transform.js` `connDeviceUnion`). The survey pair (`field_2380`↔`field_2381`) was deliberately NOT locked. Same-day follow-up: the INSTALL object's back-pointer `field_2821` (Connected To, derived from parent `field_2820`) got the same treatment — worksheet-v2 install cam detail now read-only, `connected-to-readonly.js` covers both fields on Knack-native surfaces. Steps 2–4 (retire `-recip`, reconcile sweep, collapse defensive layers) still pending — the sweep (step 3) is what self-heals already-drifted records.
- **DECISION (2026-09-11) — DELETE the camera-side fields; derive the uplink in JS. Supersedes the "read-only mirror" interim above.** Switch → camera connections are edited ONLY on the switch/NVR side (`field_1957` on SOW, `field_2380` on survey, `field_2820` on install). The camera-side back-pointers (`field_2197` / `field_2381` / `field_2821`) get REMOVED from Builder — not kept as derived copies. Do all three pairs in one pass or the pickers diverge. Accessories stay the OTHER way round (child pointer `field_2464` / `field_2853` canonical — you add a mount TO a camera); there the parent-side arrays (`field_1958`/`field_2207` on SOW, `field_2852` on install) are what gets retired. The bundle never writes those arrays; whatever still populates them lives in Make or a Builder record rule at creation time (which is why they go stale on re-parent) — find that writer and stop it.
  - **Derivation**: one shared per-render index (`parentOf(childId)`) built from every loaded record's forward list (`field_1957_raw` etc.), exposed once (e.g. on `SCW.mirrorConn`) and used by every module that reads the child field today. worksheet-v2 `init.js` (`detailConnectedDevices` union) and bid-review-v2 `transform.js` (`connDeviceUnion`) already union both sides — invert that loop. The precondition (parents loaded in the same model as the cameras) holds on every custom surface: whole SOW, whole bid review, whole install scope, whole survey request. Single-record Knack pages need no JS: show the INBOUND side of `field_1957` on the camera record.
  - **Rules whose meaning changes — rewrite deliberately**: picker "claimed by another device" (`worksheet-v2/init.js` ~1737 / ~1855 / ~2676) = some OTHER parent's list contains the camera; `worksheet-v2/warnings.js` `disconnected` = no parent lists it; `missing-connection-warn.js` the same on the survey object. Double-claim becomes representable (nothing stops two switches listing one camera): the picker must REFUSE to add a camera already listed elsewhere, and warnings must flag a double-claimed camera on the camera AND on both parents. Never let the derive silently pick one.
  - **Readers to flip (~30 files)**: `mirror-connection-sync.js` (10 `createMirror` instances — delete the `-recip` inverse handler, the reciprocal PUTs, kept-repair and converging-verify; KEEP the MDF follow-on + the `knackPutKeepalive` queue), worksheet-v2 (`config.js` `connectedDevice` slot, `card.js`, `init.js`, `warnings.js`, `bulk.js`, `change-requests.js`, `data.js`, `picker.js`), v1 `device-worksheet.js` + `connection-picker.js` + `bid-revision-inject.js`, bid-review v1/v2 (`config.js`, `transform.js`, `warnings.js`, `change-requests.js` reciprocal CRs, `init.js`), `install-as-quoted-panel.js` (quoted-vs-installed diff), `sales-change-request/` (stop tracking `field_2197` in `trackedFields`; payload + render), `sales-revision-column.js`, `sub-variant-bid.js`, `filter-mdf-idf-by-view.js` (walks `field_2381`), `missing-connection-warn.js`, `connected-device-bid-check.js`, `survey-review-grid.js`, `survey-worksheet-pdf-export.js`. DELETE `connected-to-readonly.js` outright.
  - **Builder, before deleting the fields**: replace the camera-side column on every grid/details view with the inbound display of `field_1957`; add a Count formula field (inbound connections) on each line-item object so "cameras with no uplink" filters and the ops surfaces keep working without the field; remove the field from every form, filter and record rule. The survey camera card becomes derived display (it was deliberately left editable in the 2026-08-03 pass — no longer).
  - **Make, in the SAME pass (⚠️ the scenarios hold real logic here)**: proposal→install conversion (map `field_1957` → `field_2820` with id remap; never write the child side), CO apply + swap carry-over (re-point the parent's list at the replacement record), alt-bid duplication of survey lines (`field_2380` remap), duplicate SOW (`field_1957` remap), import unique items (same records move — confirm nothing rebuilds a child pointer), any scenario that searches "which switch is this camera on" (search parents whose list CONTAINS the id instead), and the publish JSON payload (`view_3896`/`view_4140` columns) — any downstream step reading the camera-side key breaks the day the field is gone. Same audit for the accessory parent arrays: bulk add mounting box, CO add items (`accessoryIds`), conversion, carry-over.
  - **Order**: (1) reconcile ONCE, classifying every camera as consistent / listed by two parents / pointing at a parent that does not list it — a human reviews the last two classes (drift could have dropped EITHER side historically; do not assume the parent list is right for pre-fix records); (2) build the derive; (3) flip the readers; (4) Builder; (5) Make; (6) delete the fields, the dead code, and the picker's `triggerIds` 5th-arg contract (it only existed because of the reciprocal refetch race).
  - **Write cost after**: one edit = one PUT (the switch's list); a bulk re-parent = one PUT per switch. The concurrency queue only matters for MDF follow-ons and bulk actions.
...
- **Until then**: every new object needs a Hybrid card branch + careful field map (see the `view_3505` entry in `config.js` for the pattern). Each Hybrid we add is more duplication this generalization will later collapse — so prioritize it before a 4th object lands.

### 16. Survey Line Items need a parent/child (accessory + connection cascade) relationship
- **Status**: noted 2026-06-16 while building the v2 survey device worksheet (`view_3505`). The **Survey Line Item** object currently has **no** accessory parent/child denormalization and **no** mirror-sync cascade — unlike the SOW Line Item object, which has Connected Devices (`field_1957`) ↔ Connected To (`field_2197`) kept aligned by `mirror-connection-sync.js`, plus accessory `field_2464` (child→parent) ↔ `field_1958`/`field_2207` (parent→children).
- **What survey HAS today**: `field_2380` (Connected Devices, multi, on NVR/switch) ↔ `field_2381` ("Connected To", single, on cam/reader). **Correction (2026-06-16):** these ARE backed by a mirror-sync cascade — `mirror-connection-sync.js` registers a `createMirror({ VIEW_ID:'view_3505', TRIGGER_FIELD:'field_2380', CONNECTIONS_FIELD:'field_2381' })` instance (grouped by `field_2375`). The worksheet-v2 survey Connected Devices/To pickers route PUTs through view_3505 so it fires, same as the SOW `field_1957`/`field_2197` pair. What survey still LACKS is the **accessory** parent/child relationship — no accessory→parent field (`field_2464` analogue) and no `field_1958`/`field_2207` forward array — so the v2 survey card renders **no** mounting-hardware/accessory UI and maps no `accessories`/`children`/`parent`/`accessoryMatch` logical slots (see the `view_3505` config entry + card branch).
...
- **Decision (2026-09-11)**: the survey pair follows Known Issue #12 verbatim — `field_2380` (on the NVR/switch) is the only editable side; `field_2381` is to be DELETED and the camera's uplink derived in JS. `missing-connection-warn.js` and `connected-device-bid-check.js` switch to the derived uplink; the survey camera card's Connected To becomes read-only derived display.
