...
### 22. Kickoff-call booking links — auto-generate the PM × Sales scheduling link on Closed Won (idea, down the road)
- **Current pain (noted 2026-08-08)**: when a project is greenlit (closed won), contacts get a link to book a "kickoff call" with the assigned PM + the salesperson (deal owner). Today that requires a manually-created HubSpot **group meeting link for every PM × Sales combination**, a spreadsheet mapping combo → link, and a manual lookup per deal. N×M link maintenance every time the roster changes.
- **Researched 2026-08-08 — HubSpot cannot automate this natively**: there is NO public API for creating scheduling pages (the "Meetings API" only logs meeting engagements on records; even *reading* a user's link via API is a still-open idea-forum request). Round-robin links can't pin two specific required hosts ("prioritize owner" considers only the contact owner and picks a single host). Calendly is no better — its Event Type Management API creates one-on-one event types only; collective (multi-host) event types are still manual UI.
- **Recommended direction — Cal.com dynamic group links**: `cal.com/<pm-username>+<sales-username>` computes combined availability on the fly and books both calendars — the "link" is string concatenation, so nothing is ever pre-created and the spreadsheet dies. Free-plan feature, on by default; the free tier also includes two-way HubSpot sync (bookings log as meetings on the contact). Teams (~$12/user/mo annual, 2026-08 pricing) only buys branding removal + centrally-managed event-type defaults — client-facing polish, not required for the mechanism. Note: even paid "collective event types" have FIXED hosts, so dynamic links are the specific feature that solves this. Requirements: every PM + salesperson needs a Cal.com account with calendar connected and sane defaults (30 min, Meet/Zoom); store each person's Cal.com username on the HubSpot user record (or standardize usernames as first-last so it's derivable). Force settings via URL params where useful (e.g. `?duration=30`).
- **Zero-new-tools interim (same automation skeleton)**: HubSpot workflow on deal stage → Closed Won → webhook to Make → read deal owner + PM → look up combo → link in a Make data store (or the existing sheet, read by Make instead of a human) → write the URL to a `kickoff_booking_link` deal property → HubSpot workflow emails associated contacts using that property token. Kills the human lookup immediately; link creation stays manual. Later, swap the lookup step for Cal.com URL concatenation and delete the store.
- **Prerequisite either way**: the PM must exist as structured data on the deal (HubSpot user-type property) — or be readable from Knack if the greenlight actually fires Knack-side; Make can trigger from either.
- **Note**: this is CRM/scheduling-stack + Make work, not bundle code — parked here because this file is the shared backlog.
