 ...
 *
 * PATCH (2026-02-05i):
 *  - ✅ L1 footer wording EXACTLY:
 *      HEADEND: 01: Micah's Office
 *      Subtotal
 *      Discount
 *      Total
 *  - ✅ If NO discounts on the L1 group: hide "Subtotal" AND "Discount" (show only "Total")
 *  - ✅ L1 label rendered ABOVE totals inside the wide totals span (NOT in left TD)
 *  - ✅ Prevent squish: totals span cell takes 100% width and spans labor+hardware+cost
 *  - ✅ Qty stays visible on L1 footer
 */
(function () {
  'use strict';
...
/* ============================================================
   ✅ L1 footer layout:
   - label is inside the wide span cell (not the left TD)
   - span cell takes full width, so nothing gets squished
   ============================================================ */
${sel('tr.scw-subtotal--level-1 td.scw-level-total-label')} {
  display: none !important; /* we render the label inside the span */
}

/* Make the L1 span truly "own" the width */
...
/* wrapper = title (full width) + grid (right aligned) */
.scw-l1-footer {
  display: grid;
...
/* color lines */
${sel('tr.scw-subtotal--level-1')} .scw-l1-totals-grid__label.scw-l1-totals-grid__sub,
${sel('tr.scw-subtotal--level-1')} .scw-l1-totals-grid__value.scw-l1-totals-grid__sub {
...
${sel('tr.scw-subtotal--level-1')} .scw-l1-totals-grid__label.scw-l1-totals-grid__total,
${sel('tr.scw-subtotal--level-1')} .scw-l1-totals-grid__value.scw-l1-totals-grid__total {
...
/* ✅ force L1 subtotal row background to apply to all TDs */
${sel('tr.scw-level-total-row.scw-subtotal--level-1')} { background: RGB(7, 70, 124, 1) !important; }
${sel('tr.scw-level-total-row.scw-subtotal--level-1 td')} { background: inherit !important; }
...
`;
  ...
  function buildSubtotalRow(ctx, caches, {
    ...
    // Total = gross + signed discount (supports either sign convention)
    const total = cost + (hasDiscount ? discount : 0);

    // ✅ never apply hideQtyCost to L1
    ...
    // ✅ L1 footer: label ABOVE totals (inside wide span) + conditional lines
    if (level === 1) {
      // hide entire L1 footer if $0
      ...
      // span labor across labor+hardware+cost and hide the other two
      $hardwareCell.css('display', 'none');
      $costCell.css('display', 'none');

      $laborCell.attr('colspan', '3').addClass('scw-l1-totals-span');

      const title = escapeHtml(leftText);

      // If no discount => ONLY show Total line (hides Subtotal + Discount)
      const gridHtml = hasDiscount
        ? `
          <div class="scw-l1-totals-grid">
            <div class="scw-l1-totals-grid__label scw-l1-totals-grid__sub">Subtotal</div>
            <div class="scw-l1-totals-grid__value scw-l1-totals-grid__sub">${escapeHtml(formatMoney(cost))}</div>

            <div class="scw-l1-totals-grid__label scw-l1-totals-grid__disc">Discount</div>
            <div class="scw-l1-totals-grid__value scw-l1-totals-grid__disc">–${escapeHtml(formatMoneyAbs(discount))}</div>

            <div class="scw-l1-totals-grid__label scw-l1-totals-grid__total">Total</div>
            <div class="scw-l1-totals-grid__value scw-l1-totals-grid__total">${escapeHtml(formatMoney(total))}</div>
          </div>
        `
        : `
          <div class="scw-l1-totals-grid">
            <div class="scw-l1-totals-grid__label scw-l1-totals-grid__total">Total</div>
            <div class="scw-l1-totals-grid__value scw-l1-totals-grid__total">${escapeHtml(formatMoney(cost))}</div>
          </div>
        `;

      $laborCell.html(`
        <div class="scw-l1-footer">
          <div class="scw-l1-footer__title">${title}</div>
          ${gridHtml}
        </div>
      `);

      // left label TD not used anymore
      $row.find('td.scw-level-total-label').empty();
    }
    ...
    for (const anchorEl of anchors) {
      ...
      for (const item of items) {
        const $row = buildSubtotalRow(ctx, caches, {
          ...
          // ✅ IMPORTANT: L1 title should be JUST the L1 label (no "— Subtotal")
          labelOverride: item.level === 1 ? `${item.label}` : null,
